...
export function initCommand(): Command {
	return new Command("init")
		...
		.option(
			"--from-claude-dir <path>",
			"migrate from existing .claude/ directory",
		)
		.action(async (name: string, options: { fromClaudeDir?: string }) => {
			...
			if (options.fromClaudeDir) {
				logger.error(
					"--from-claude-dir migration not yet implemented. Coming in a later release.",
				);
				process.exit(1);
			}
			...
			await writeTextFile(
				join(target, "skills/general/my-skill/SKILL.md"),
				TEMPLATE_SKILL,
			);
