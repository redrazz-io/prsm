...
import { trackGeneratedFile, copyTrackedFile, cleanGeneratedFiles } from "../utils/generated-files";
...
export class CodexAdapter implements RuntimeAdapter {
  ...
  async compileSkill(skill: ResolvedSkill, outputBase: string, output?: OutputConfig): Promise<void> {
    ...
    const destSkillDir = join(outputBase, output?.skills ?? ".agents/skills", dirName);
    ...
    // A skill is a directory: copy its sibling support files too.
