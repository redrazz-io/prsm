...
let tmp: string;
beforeEach(async () => {
	tmp = await mkdtemp(join(tmpdir(), "prsm-eject-"));
});
afterEach(async () => {
...
`;
		...
		expect(after).toContain(
			"# Platform team's hub manifest — owned by @platform",
		);
	...
	it("dedups permissions (AE3): preset perms appended, existing kept once", async () => {
		const presetDir = join(tmp, "presets/test-preset");
		await setupPreset(presetDir, {
			permissions: ["Bash(git:*)", "Read(./docs/**)"],
		});
...
`;
		...
		const after = parseYaml<{ permissions: string[] }>(
	...
	it("local gains preset hook when local has no value for that event", async () => {
		const presetDir = join(tmp, "presets/test-preset");
		await setupPreset(presetDir, {
			hooks: { "session-start": "./hooks/session-start.sh" },
		});
...
`;
		...
		const after = parseYaml<{ hooks: Record<string, string> }>(
			await readTextFile(join(tmp, "prsm.yaml")),
		);
	...
	it("local hook wins when present (last-wins per event)", async () => {
		const presetDir = join(tmp, "presets/test-preset");
		await setupPreset(presetDir, {
			hooks: { "session-start": "./hooks/preset.sh" },
		});
...
`;
	...
	it("existing dependency keys unchanged (key-level merge)", async () => {
		const presetDir = join(tmp, "presets/test-preset");
		await setupPreset(presetDir, {
			dependencies: { kubectl: ">=1.28", helm: ">=3.10" },
		});
...
`;
		...
		const after = parseYaml<{ dependencies: Record<string, string> }>(
			await readTextFile(join(tmp, "prsm.yaml")),
		);
...
`;
		...
		const after = parseYaml<{ extends: string[] }>(
			await readTextFile(join(tmp, "prsm.yaml")),
		);
		expect(after.extends).toEqual([]);
	});
