...
let tmp: string;
beforeEach(async () => {
	tmp = await mkdtemp(join(tmpdir(), "prsm-dep-"));
});
afterEach(async () => {
...
function makeSkill(
	name: string,
	deps: Record<string, { source: string; required: boolean }>,
): ResolvedSkill {
	return {
		name,
		category: "test",
		frontmatter: {
			...
			dependencies: Object.fromEntries(
				Object.entries(deps).map(([k, v]) => [
					...
					{
						type: "skill" as const,
						source: v.source as "local" | "remote",
						required: v.required,
					},
				]),
			),
...
describe("validateDependencyPresence", () => {
	it("passes when required local dep exists", async () => {
		...
		await writeTextFile(
			join(tmp, "skills/test/dep-skill/SKILL.md"),
			"---\nname: dep-skill\ndescription: x\n---\ncontent",
		);
		const skill = makeSkill("my-skill", {
			"dep-skill": { source: "local", required: true },
		});
	...
	it("fails when required local dep is missing", async () => {
		const skill = makeSkill("my-skill", {
			"missing-dep": { source: "local", required: true },
		});
		const errors = await validateDependencyPresence([skill], tmp);
	...
	it("ignores optional local dep that is missing", async () => {
		const skill = makeSkill("my-skill", {
			"opt-dep": { source: "local", required: false },
		});
		const errors = await validateDependencyPresence([skill], tmp);
		expect(errors).toHaveLength(0);
	});
	...
	it("ignores remote deps (not locally checkable)", async () => {
		const skill = makeSkill("my-skill", {
			"remote-dep": { source: "remote", required: true },
		});
		const errors = await validateDependencyPresence([skill], tmp);
		expect(errors).toHaveLength(0);
	});
