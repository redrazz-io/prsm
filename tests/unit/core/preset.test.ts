...
describe("computePresetContentHash", () => {
	...
	it("normalizes line endings (CRLF and LF produce identical hash)", async () => {
		...
		try {
			await writeFile(
				join(dirA, "preset.yaml"),
				"name: x\nversion: 1.0.0\n",
				"utf-8",
			);
			await writeFile(
				join(dirB, "preset.yaml"),
				"name: x\r\nversion: 1.0.0\r\n",
				"utf-8",
			);
	...
	it("rejects symlinked SKILL.md (C3 integrity-bypass regression)", async () => {
		...
		try {
			...
			await symlink(
				...
				join(tmp, "skills/cat/foo/SKILL.md"),
			);
			await expect(computePresetContentHash(tmp)).rejects.toThrow(
	...
	it("rejects symlinked hook script (C3 integrity-bypass regression)", async () => {
		...
		try {
			...
			await symlink(
				join(targetDir, "hook.sh"),
				join(tmp, "hooks/session-start.sh"),
			);
			await expect(computePresetContentHash(tmp)).rejects.toThrow(
				/Symbolic links are not allowed/,
			);
